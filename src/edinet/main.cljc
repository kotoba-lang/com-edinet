(ns edinet.main
  "Kotodama WASM entrypoint for the Edinet clean-room actor (L4) — Clojure port.
  ...
  facts (`edinet.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "edinet")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"edinet.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "edinet-compat" :tier tier :entities entities} 200])
...
(defn start! [] :edinet-compat/ready)
